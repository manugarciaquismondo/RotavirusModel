...
mu=0*5/InitialTotalPop; % 5/20000
% mu=5/20000;
...
totalIterations=numberOfYears*iterationsPerYear;


Store_m0=zeros(1,totalIterations);
Store_s0=zeros(1,totalIterations);
Store_i0=zeros(1,totalIterations);
Store_r0=zeros(1,totalIterations);
Store_v1=zeros(1,totalIterations);
Store_s1=zeros(1,totalIterations);
Store_i1=zeros(1,totalIterations);
Store_r1=zeros(1,totalIterations);
Store_v2=zeros(1,totalIterations);
Store_s2=zeros(1,totalIterations);
Store_i2=zeros(1,totalIterations);
Store_r2=zeros(1,totalIterations);
Store_v3=zeros(1,totalIterations);
Store_s3=zeros(1,totalIterations);
Store_i3=zeros(1,totalIterations);
Store_r3=zeros(1,totalIterations);
Store_v4=zeros(1,totalIterations);
Store_s4=zeros(1,totalIterations);
Store_i4=zeros(1,totalIterations);
Store_r4=zeros(1,totalIterations);
...
%dI0/dt = x(3)
I0= i0  + (1-(1/183))*kappa*lambda*s0*(i0+i1+i2+i3+i4) - (1/183)*i0 - (1-(1/183))*gamma*i0;
...
%dV1/dt = x(5)
V1= v1  + nu*(1/183)*r0  + nu*(1/183)*s0  + nu*(1-(1/183))*s1  + nu*(1-(1/183))*r1...
    - (1/183)*v1  - (1-(1/183))*kappa*(1-lambda)*omega_V*v1*(i0+i1+i2+i3+i4);
...
%DV2/dt = x(9)
V2= v2 + nu*(1/183)*r1 + nu*(1/183)*s1 + nu*(1-(1/365))*s2  + nu*(1-(1/365))*r2...
    - (1/365)*v2  - (1-(1/365))*kappa*(1-lambda)*omega_V*v2*(i0+i1+i2+i3+19);
...
%DV3/dt = x(13)
V3= v3 + nu*(1/365)*r2 + nu*(1/365)*s2 + nu*(1-(1/1095))*s3  + nu*(1-(1/1095))*r3...
    -(1/1095)*v3 - (1-(1/1095))*kappa*(1-lambda)*omega_V*v3*(i0+i1+i2+i3+i4);
...
%DR3/dt = x(16)
R3= r3  + (1-(1/1095))*iota*gamma*i3  + (1-nu)*(1/365)*r2  + (1/1095)*iota*gamma*i2...
    - (1/1095)*r3  - (1-(1/1095))*kappa*omega_R*(1-lambda)*r3*(i0+i1+i2+i3+i4)...
    - (1-(1/1095))*nu*r3;

%DV4/dt = x(17)
V4= v4 + nu*(1/1095)*r3 + nu*(1/1095)*s3 + nu*(1-(1/1095))*s4  + nu*(1-(1/1095))*r4...
    - 0*(1/1095)*v4 -(1-(1/1095))*kappa*(1-lambda)*omega_V*v4*(i0+i1+i2+i3+i4);
...
S4= s4 + (1-nu)*(1-kappa*lambda*(i0+i1+i2+i3+i4))*s3/1095 + v3/1095 ...
    ...
    -(1-(1/1095))*kappa*lambda*s4*(i0+i1+i2+i3+i4) -0*(s4/1095) - (1-(1/1095))*nu*s4 ...
    +(1/1095)*(1-iota)*gamma*i3;

%DI4/dt = x(19)
I4 = i4 +(1-(1/1095))*kappa*lambda*s4*(i0+i1+i2+i3+i4)+(s3/1095)*kappa*lambda*(i0+i1+i2+i3+i4)...
    + (1/1095)*(1-gamma)*i3 -0*(1/1095)*i4  - (1-(1/1095))*gamma*i4 ;

%DR4/dt = x(20)
R4= r4  + (1-(1/1095))*iota*gamma*i4  + (1-nu)*(1/1095)*r3  + (1/1095)*iota*gamma*i3...
    - 0*(1/1095)*r4 - (1-(1/1095))*kappa*omega_R*(1-lambda)*r4*(i0+i1+i2+i3+i4)...
    - (1-(1/1095))*nu*r4;
